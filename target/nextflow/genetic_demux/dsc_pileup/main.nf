// dsc_pileup 461-rename-workflows_build
// 
...
meta = [
    ...
    "resources" : [
      {
        ...
        "path" : "script.sh",
        "is_executable" : true,
        "parent" : "file:/home/runner/work/openpipeline/openpipeline/src/genetic_demux/dsc_pileup/"
      }
    ],
    "description" : "\\"dsc-pileup is a software tool to pileup reads and corresponding base quality for each overlapping SNPs and each barcode. By using pileup files, it would allow us to run demuxlet/freemuxlet pretty fast multiple times without going over the BAM file again.\n",
    "test_resources" : [
      {
        "type" : "bash_script",
        "path" : "test.sh",
  ...
  "info" : {
    "config" : "/home/runner/work/openpipeline/openpipeline/src/genetic_demux/dsc_pileup/config.vsh.yaml",
    "platform" : "nextflow",
...
$( if [ ! -z ${VIASH_PAR_TAG_GROUP+x} ]; then echo "${VIASH_PAR_TAG_GROUP}" | sed "s#'#'\\"'\\"'#g;s#.*#par_tag_group='&'#" ; else echo "# par_tag_group="; fi )
$( if [ ! -z ${VIASH_PAR_TAG_UMI+x} ]; then echo "${VIASH_PAR_TAG_UMI}" | sed "s#'#'\\"'\\"'#g;s#.*#par_tag_umi='&'#" ; else echo "# par_tag_umi="; fi )
$( if [ ! -z ${VIASH_PAR_EXCLUDE_FLAG+x} ]; then echo "${VIASH_PAR_EXCLUDE_FLAG}" | sed "s#'#'\\"'\\"'#g;s#.*#par_exclude_flag='&'#" ; else echo "# par_exclude_flag="; fi )
$( if [ ! -z ${VIASH_PAR_VCF+x} ]; then echo "${VIASH_PAR_VCF}" | sed "s#'#'\\"'\\"'#g;s#.*#par_vcf='&'#" ; else echo "# par_vcf="; fi )
$( if [ ! -z ${VIASH_PAR_SM+x} ]; then echo "${VIASH_PAR_SM}" | sed "s#'#'\\"'\\"'#g;s#.*#par_sm='&'#" ; else echo "# par_sm="; fi )
$( if [ ! -z ${VIASH_PAR_SM_LIST+x} ]; then echo "${VIASH_PAR_SM_LIST}" | sed "s#'#'\\"'\\"'#g;s#.*#par_sm_list='&'#" ; else echo "# par_sm_list="; fi )
$( if [ ! -z ${VIASH_PAR_SAM_VERBOSE+x} ]; then echo "${VIASH_PAR_SAM_VERBOSE}" | sed "s#'#'\\"'\\"'#g;s#.*#par_sam_verbose='&'#" ; else echo "# par_sam_verbose="; fi )
$( if [ ! -z ${VIASH_PAR_VCF_VERBOSE+x} ]; then echo "${VIASH_PAR_VCF_VERBOSE}" | sed "s#'#'\\"'\\"'#g;s#.*#par_vcf_verbose='&'#" ; else echo "# par_vcf_verbose="; fi )
$( if [ ! -z ${VIASH_PAR_SKIP_UMI+x} ]; then echo "${VIASH_PAR_SKIP_UMI}" | sed "s#'#'\\"'\\"'#g;s#.*#par_skip_umi='&'#" ; else echo "# par_skip_umi="; fi )
$( if [ ! -z ${VIASH_PAR_CAP_BQ+x} ]; then echo "${VIASH_PAR_CAP_BQ}" | sed "s#'#'\\"'\\"'#g;s#.*#par_cap_bq='&'#" ; else echo "# par_cap_bq="; fi )
...
$( if [ ! -z ${VIASH_PAR_MIN_TD+x} ]; then echo "${VIASH_PAR_MIN_TD}" | sed "s#'#'\\"'\\"'#g;s#.*#par_min_td='&'#" ; else echo "# par_min_td="; fi )
$( if [ ! -z ${VIASH_PAR_EXCL_FLAG+x} ]; then echo "${VIASH_PAR_EXCL_FLAG}" | sed "s#'#'\\"'\\"'#g;s#.*#par_excl_flag='&'#" ; else echo "# par_excl_flag="; fi )
$( if [ ! -z ${VIASH_PAR_GROUP_LIST+x} ]; then echo "${VIASH_PAR_GROUP_LIST}" | sed "s#'#'\\"'\\"'#g;s#.*#par_group_list='&'#" ; else echo "# par_group_list="; fi )
$( if [ ! -z ${VIASH_PAR_MIN_TOTAL+x} ]; then echo "${VIASH_PAR_MIN_TOTAL}" | sed "s#'#'\\"'\\"'#g;s#.*#par_min_total='&'#" ; else echo "# par_min_total="; fi )
$( if [ ! -z ${VIASH_PAR_MIN_UNIQ+x} ]; then echo "${VIASH_PAR_MIN_UNIQ}" | sed "s#'#'\\"'\\"'#g;s#.*#par_min_uniq='&'#" ; else echo "# par_min_uniq="; fi )
$( if [ ! -z ${VIASH_PAR_MIN_SNP+x} ]; then echo "${VIASH_PAR_MIN_SNP}" | sed "s#'#'\\"'\\"'#g;s#.*#par_min_snp='&'#" ; else echo "# par_min_snp="; fi )
...
meta["defaults"] = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "openpipelines-bio/genetic_demux_dsc_pileup",
    "tag" : "main_build"
  },
  "label" : [
    "midmem",
